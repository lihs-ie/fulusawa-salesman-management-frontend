...
import { DailyReport, Criteria } from 'domains/daily-report';
import { Builder } from 'tests/factories/common';
import { DailyReportFactory, CriteriaFactory } from 'tests/factories/domains/daily-report';
import { Resource, Type as CommonType } from 'tests/mock/upstreams/common';

import { DailyReportsMedia } from '../../media';
...
export class ListResource extends Resource<CommonType, Overrides, Query> {
  ...
  private readonly media: DailyReportsMedia;
  ...
  public constructor(type: CommonType, overrides?: Overrides) {
    super(
      type,
      overrides ?? {
        model: Builder.get(DailyReportFactory).buildList(Math.floor(Math.random() * 20) + 1),
      }
    );
  ...
  public matches(request: Request, uri: string): boolean {
    ...
    if (
      query.isSubmitted !==
      (this.identifier.isSubmitted !== null ? String(this.identifier.isSubmitted) : undefined)
    ) {
      return false;
    }
  ...
  private createMedia(overrides?: Overrides): DailyReportsMedia {
    if (List.isList(overrides?.model)) {
      return new DailyReportsMedia(overrides.model);
    }

    return new DailyReportsMedia(overrides?.media);
  }
}
