...
export type Overrides = VisitIdentifier;

export class DeleteResource extends Resource<CommonType, Overrides, {}> {
  private static readonly CODE_PREFIX = 'fcm-backend/visits/delete';

  public constructor(type: CommonType, overrides?: Overrides) {
    super(type, overrides ?? Builder.get(VisitIdentifierFactory).build());
  }
  ...
  public matches(request: Request, uri: string): boolean {
    ...
    if (!uri.startsWith(`/visits/${this.overrides.value}`)) {
      return false;
    }
